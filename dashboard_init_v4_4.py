...
def CF_boxplot(data,plantName,unitName,yearRange):
    """
    Function generating our first visualisation.
...
def Norm_Em(data,plantName,unitName,yearRange,RM_window,CO2_on,SO2_on,NOX_on):
    """
    Function generating the second visualisation
...
def EmvCF(data,plantName,unitName,yearRange,Nplots,CO2scale,SO2scale):
    """
    Function generating our third visualisation
...
def emissionHistograms(data,plantName,unitName,yearRange,eff_on,CapFac_on,CO2e_on,CO2_on,SO2_on,NOX_on):
    """
    Function for our fourth visualisation
...
def interactive_plots(data,plantName,unitName):
    ...
    printmd('This plot displays a time series of CO2, SO2, and NOx emissions. It tells a story of how the emissions of a unit changes over time. Leads to insights about upgrades, maintenance, extreme events, etc.')
    ...
    printmd('You may choose below the time range to display as well as the number of years. Additionally, you may widen the display window if needed. The scaling value corresponds to how many times the median value the plot goes up to.')
